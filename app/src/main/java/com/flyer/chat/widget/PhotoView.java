...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    public InfoPhotoView getInfo() {
        ...
        return new InfoPhotoView(rect, local, mImgRect, mWidgetRect, mScale, mDegrees, mScaleType);
    }
    ...
    public void animaFrom(InfoPhotoView info) {

        if (isInit) {
            reset();

            InfoPhotoView mine = getInfo();
    ...
    public void animaTo(InfoPhotoView info, Runnable completeCallBack) {
        if (isInit) {
            mTranslate.stop();
